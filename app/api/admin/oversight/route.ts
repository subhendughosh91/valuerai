...
export async function GET(request: Request) {
  ...
  const { data: users, error: usersError } = await context.supabase.from("profiles").select("id,display_name,phone,address,state_code,country,created_at").eq("role", "USER").eq("state_code", stateCode).order("created_at", { ascending: false });
