...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await context.supabase.from("valuations").update({ approved_data: approved, valuation_rule_id: valuationRule.id, land_rule_id: landRule.id, approved_at: new Date().toISOString() }).eq("id", id);
