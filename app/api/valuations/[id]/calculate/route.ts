...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params; const { data: valuation } = await context.supabase.from("valuations").select("id,status,approved_data,custom_instructions").eq("id", id).single();
  ...
  const valuationInput = await request.json();
  const output = calculateTripuraValuation(valuationInput);
  const { data, error } = await context.supabase.from("valuation_calculations").upsert({ valuation_id: id, input_snapshot: { approvedData: valuation.approved_data ?? {}, valuationInput, customInstructions: valuation.custom_instructions }, output }).select().single();
