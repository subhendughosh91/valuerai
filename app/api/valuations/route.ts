...
export async function GET() {
  ...
  const { data, error } = await context.supabase.from("valuations").select("id,reference_no,property_label,status,created_at,updated_at,reports(id)").neq("status", "DISCARDED").order("updated_at", { ascending: false });
